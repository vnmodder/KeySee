...
export const colorsBgLight: { [key: string]: string } = {
...
export const colorsText: { [key: string]: string } = {
...
export const colorsOutline: { [key: string]: string[] } = {
...
type Color = keyof typeof colorsBgLight;

interface ButtonColors {
  ring: { [key in Color]: string };
  active: { [key in Color]: string };
  bg: { [key in Color]: string };
  bgHover: { [key in Color]: string };
  borders: { [key in Color]: string };
  text: { [key in Color]: string };
  outlineHover: { [key in Color]: string };
}

export const getButtonColor = (color: Color, isOutlined: boolean, hasHover: boolean, isActive: boolean = false): string[] => {
  ...
  const validColors: Color[] = ['white', 'whiteDark', 'lightDark'];
  const isOutlinedProcessed = isOutlined && validColors.indexOf(color) < 0;
